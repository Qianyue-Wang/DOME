...
#这里定义了系列基本的chat函数，是基于api调用

# def chat(prompt):
   
#     client = ZhipuAI(api_key="d39935fd2d9f65c13a99c43da8fa1d6a.NnBTo13yIFmhxNpA") # 填写您自己的APIKey
#     messages = [
...
def chat(prompt):
    dashscope.api_key = "sk-b43e4ff9a8df477696b8e585186ee8e4"
    messages = [
        {"role": "system",
             "content": "You are a Knowledge Graph expert and are good at extracting information from the Knowledge Graph. You are helping a user to extract information from the Knowledge Graph."},
        {'role': 'user', 'content': prompt}]
...
def chate(prompt):
    cdashscope.api_key = "sk-b43e4ff9a8df477696b8e585186ee8e4"
    messages = [
        {"role": "system",
...
def find_shortest_path(start_entity_name, end_entity_name,candidate_list,title):
...
def get_history_triples():
            path='/mnt/cephfs/home/wangqianyue/LTG/MindMap-main/data/KGc.csv'
            df = pd.read_csv(path)
    ...
    if len(match_kg) != 1 or 0:
                ...
                while 1:
                    ...
                    while candidate_entity != []:
                        ...
                        paths,exist_entity = find_shortest_path(start_entity, end_entity,candidate_entity,title)
...
def evaluator(outline,triples,title):
    if len(triples)<16:
        ...
        for i in triples:
            ...
            #检查res的输出是否规范，检查res输出是否不含/,如果含有/，则重新评估
            score=0
            count=4
